...
export const transformDashboardStats = (raw, complianceRaw = null, generalStats = null, dailySummary = null) => {
  ...
  const points = compliancePoints.map((p) => {
    ...
    return {
      ...
      warnings_count: 0,
      flow_exceeded_count: p.flow_history?.count ?? null,
      flow_near_limit_count: p.near_limit_history?.count ?? null,
      flow_analysis_days: 0,
      flow_history: p.flow_history || null,
      near_limit_history: p.near_limit_history || null,
      compliance_warning: {
